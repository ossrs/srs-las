...
public:
    ...
    SrsLiveConsumer(SrsLiveSource* s, int64_t lasspts = 0, bool onlyAudio = false);
#else
...
public:
     ...
     */
    virtual srs_error_t dump(SrsLiveConsumer* consumer, bool dm, bool ds, int64_t lasspts = 0);
    //@param begin_index the index of first msg to dump in cache_vec
    virtual srs_error_t do_dump(SrsLiveConsumer* consumer,  bool dm, bool ds, int begin_index);
...
public:
    ...
    virtual srs_error_t create_consumer(SrsLiveConsumer*& consumer, int64_t lasspts = 0, bool only_audio = false);
    // Dumps packets in cache to consumer.
    // @param ds, whether dumps the sequence header.
    // @param dm, whether dumps the metadata.
    // @param dg, whether dumps the gop cache.
    // @param lasspts, @startPts in las1.0
    virtual srs_error_t consumer_dumps(SrsLiveConsumer* consumer, bool ds = true, bool dm = true, bool dg = true, int64_t lasspts = 0);
#else
    // Create consumer
    // @param consumer, output the create consumer.
